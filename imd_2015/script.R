...
df <- read_csv("http://opendatacommunities.org/downloads/cube-table?uri=http%3A%2F%2Fopendatacommunities.org%2Fdata%2Fsocietal-wellbeing%2Fimd%2Findices") %>% 
...
df <- filter(df, lsoa11cd %in% lsoa_codes)

# ------- write results
write_csv(df, "data/IMD_2015.csv")
