...
# filter data ---------------------------
sites <- filter(polygons, BUILDGTHEM == "Education" &
                  CLASSIFICA %in% c("Secondary Education", "Non State Secondary Education")) %>% 
  mutate(type = case_when(
    CLASSIFICA == "Secondary Education" ~ "State",
    CLASSIFICA == "Non State Secondary Education" ~ "Non state"
  ))
...
st_write(sf_gm, "gm_secondary_schools.geojson", driver = "GeoJSON")
st_write(sf_trafford, "trafford_secondary_schools.geojson", driver = "GeoJSON")
