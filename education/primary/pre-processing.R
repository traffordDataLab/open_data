...
sites <- filter(polygons, BUILDGTHEM == "Education" &
                  CLASSIFICA %in% c("Primary Education", "Non State Primary Education")) %>% 
  mutate(type = case_when(
    CLASSIFICA == "Primary Education" ~ "State",
    CLASSIFICA == "Non State Primary Education" ~ "Non state"
  ))
...
st_write(sf_gm, "gm_primary_schools.geojson", driver = "GeoJSON")
st_write(sf_trafford, "trafford_primary_schools.geojson", driver = "GeoJSON")